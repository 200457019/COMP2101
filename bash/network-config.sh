...
echo '=========================='
echo '========= Task 1 ========='
commandOne=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
localAreaNetworkAddresss=$(ip a s $commandOne |awk '/inet /{gsub(/\/.*/,"");print $2}')
localAreaNetworkName=$(getent hosts $localAreaNetworkAddresss | awk '{print $2}')
ourExternalIpAddress=$(curl -s icanhazip.com)
outExternalName=$(getent hosts $ourExternalIpAddress | awk '{print $2}')

cat <<EOF
Hostname        : $(hostname)
LAN Address     : $localAreaNetworkAddresss
LAN Hostname    : $localAreaNetworkName
External IP     : $ourExternalIpAddress
External Name   : $outExternalName
EOF
echo '=========================='
...
echo '========= Task 2 ========='
ourRouterAddress=$(ip r | awk '{print $3; exit}')
ourRouterName=$(getent hosts $ourRouterAddress | awk '{print $2}')

cat <<EOF
Router Address  : $ourRouterAddress
Router Hostname : $ourRouterName
EOF
echo '========== END ==========='
